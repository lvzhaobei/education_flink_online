...
import com.baiyi.education.model._
...
object DImJoinData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stateBackend: StateBackend = new RocksDBStateBackend("hdfs://192.168.155.179:8020/flink/checkpoint")
    ...
    val dwdmemberStream = env.addSource(dwdmemberSource).assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor[DwdMember](Time.seconds(10)) {
      override def extractTimestamp(element: DwdMember): Long = element.register.toLong
    })
    ...
    // 用户表现关联注册表 以用户表为主表  用cogroup 实现left join

    val dwdmemberLeftJoinRegtyeStream= dwdmemberStream.coGroup(dwdmemberregtypeStream).where(item=>item.uid +"_"+item.dn)
...
class MemberLeftJoinRegtype extends  CoGroupFunction[DwdMember,DwdMemberRegtype,String]{
  override def coGroup(first: lang.Iterable[DwdMember], second: lang.Iterable[DwdMemberRegtype], out: Collector[String]): Unit ={
    ...
    while(leftIterator.hasNext){
      ...
      jsonObject.put("lastlogin",dwdMember.password)
