...
import com.baiyi.education.model.GlobalConfig
...
/**
  * flink 异步 io jopin hbase
  *
  * 根据官网推荐  异步io写法 使用异步客户端 如果没有异步客户端自己可以写多线程实现
  */

class DimHbaseAsyncFunction extends RichAsyncFunction[String,String]{
  var executorService:ExecutorService = _
  var cache:Cache[String,String]= _
  //使用opentsdb 的 异步客户端  http://opentsdb.github.io/asynchbase/javadoc/index.html  //这个源码指向是hbase1-2之间的版本适用
  override def open(parameters: Configuration): Unit = {
   ...
   executorService= Executors.newFixedThreadPool(12)
    ...
    cache=CacheBuilder.newBuilder()
      ...
      .expireAfterAccess(2,TimeUnit.HOURS) //设置缓存2小时过期
    ...
    if(!"".equals(vip_id)){
      ...
      if(vipDetail==null || "".equals(vipDetail)){
       ...
       val table= connection.getTable(TableName.valueOf("education:dwd_membervip"))
        ...
        for(cell <- result.rawCells()){
          val colName=Bytes.toString(CellUtil.cloneQualifier(cell))
          val value=Bytes.toString(CellUtil.cloneQualifier(cell))
          colName match {
